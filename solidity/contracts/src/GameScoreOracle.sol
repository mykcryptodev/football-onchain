...
contract GameScoreOracle is ConfirmedOwner, FunctionsClient {
    ...
    string public constant SOURCE =
        ...
        "const packedQuarterScores=(BigInt(homeQ1)<<248n)|(BigInt(homeQ2)<<240n)|(BigInt(homeQ3)<<232n)|(BigInt(homeF)<<224n)|(BigInt(awayQ1)<<216n)|(BigInt(awayQ2)<<208n)|(BigInt(awayQ3)<<200n)|(BigInt(awayF)<<192n);"
        "const packedQuarterDigits=(BigInt(homeQ1LastDigit)<<252n)|(BigInt(homeQ2LastDigit)<<248n)|(BigInt(homeQ3LastDigit)<<244n)|(BigInt(homeFLastDigit)<<240n)|(BigInt(awayQ1LastDigit)<<236n)|(BigInt(awayQ2LastDigit)<<232n)|(BigInt(awayQ3LastDigit)<<228n)|(BigInt(awayFLastDigit)<<224n);"
        "let packedResult=[BigInt(qComplete),BigInt(gameCompleted?1:0),BigInt(scoreChanges.length),packedQuarterScores,packedQuarterDigits];"
    ...
    string public constant SCORE_CHANGES_SOURCE =
        ...
        "function numberToUint256(num){const hex=BigInt(num).toString(16);return hex.padStart(64,'0')}"
        "let packedResult=[BigInt(scoreChanges.length)];"
