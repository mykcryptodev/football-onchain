/**
 * Pure ESPN fetch/parse/pack logic — no CRE imports, unit-testable with plain bun/node.
 * Ports the four JS sources from the legacy GameScoreOracle (Chainlink Functions).
...
export const fetchJson = async (url: string): Promise<any> => {
  const resp = await fetch(url, { headers: { "Content-Type": "application/json", "User-Agent": "curl/8.0" } })
...
export const buildGameScoresPayload = (data: any, gameId: bigint): `0x${string}` => {
  ...
  if (!homeTeam || !awayTeam) throw new Error("Unable to find home or away team")

  const gameCompleted = data.header.competitions[0].status.type.completed || false
  const qComplete = gameCompleted ? 100 : (data.header.competitions[0].status.period || 1) - 1
  ...
  let packedScores = 0n
  for (let i = 0; i < 8; i++) packedScores |= BigInt(scores[i]) << BigInt(248 - i * 8)

  const totalScoreChanges = (data.scoringPlays || []).length

  return encodeAbiParameters(GAME_SCORES_PARAMS, [
    0, gameId, qComplete, gameCompleted, totalScoreChanges, packedScores, packedDigits,
  ])
}

// ---------- score changes (reportType 1) ----------

export const buildScoreChangesPayload = (data: any, gameId: bigint): `0x${string}` => {
  ...
  for (let i = 0; i < Math.min(scoreChanges.length, 64); i += 8) {
    ...
    for (let j = 0; j < 8 && i + j < scoreChanges.length; j++) {
      ...
      const packedChange = (lastDigit(c.homeScore || 0) << 4n) | lastDigit(c.awayScore || 0)
      word |= packedChange << BigInt(j * 32)
    }
    packed.push(word)
  }
  return encodeAbiParameters(SCORE_CHANGES_PARAMS, [1, gameId, scoreChanges.length, packed])
}

// ---------- week games (reportType 2) ----------

export const buildWeekGamesPayload = (data: any, weekId: bigint): `0x${string}` => {
  const events = (data?.events || []).sort((a: any, b: any) => a.id.localeCompare(b.id))
  const packed: bigint[] = []
...
export const buildWeekResultsPayload = async (
  ...
  const events = (scoreboardData?.events || []).sort((a: any, b: any) => a.id.localeCompare(b.id))
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (v.status.type.completed && home && away) {
      if (Number(home.score) > Number(away.score)) packedResults |= 1n << BigInt(i)
      completed++
    }
  ...
  return encodeAbiParameters(WEEK_RESULTS_PARAMS, [
    3, weekId, allCompleted, events.length, packedResults, totalPoints, tiebreakerGameId,
  ])
}

export const calculateWeekId = (year: bigint, seasonType: number, weekNumber: number): bigint =>
  (year << 16n) | (BigInt(seasonType) << 8n) | BigInt(weekNumber)

export const weekIdToParams = (weekId: bigint): { year: bigint; seasonType: number; weekNumber: number } => ({
  year: weekId >> 16n,
