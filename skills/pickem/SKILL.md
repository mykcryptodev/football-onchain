...
Before sending, re-read your own draft: if the line that should carry `links.picks` has no `https://` URL in it, you dropped the link — fix it, don't send it anyway. A reply that promises a link and then omits it is worse than no reply. Never truncate games instead of linking out. After verification, use the confirmed entry response's `share.text` verbatim as the final line and attach `share.imageUrl` as the reply image on X. Do not replace either with the transaction/explorer URL, omit them, or paraphrase them. Before sending, verify `share.text` contains the entry `url` and `share.imageUrl` is an HTTPS URL. If the surface cannot attach remote media, send `share.text` with its entry URL as the fallback so X can unfurl the same OG image. The entry page displays actual onchain picks without connecting a wallet. A template or draft is never described as submitted.
