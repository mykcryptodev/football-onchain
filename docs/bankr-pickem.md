...
The verified `GET /api/bankr/contests/{id}/entries?tokenId={tokenId}` response wraps the entry in `entries[]`. Each entry's `share` includes `text`, `imageUrl`, `imageAlt`, `markdown` (inline image plus share text), `fallbackText` (entry link plus direct image link), and `status` (`"pending" | "ready" | "failed"`). These are app response fields, not a Bankr-specific media API schema.

`share.imageUrl` (`/api/og/pickem/{id}/picks?tokenId={tokenId}`) is a pre-generated image, not rendered on the fly — the route only reads a Redis status record and redirects to a persisted Vercel Blob URL, and never touches the blockchain or ESPN. Hitting this single-tokenId endpoint (from the Bankr route or the `/pickem/{id}/entries/{tokenId}` page) is also what queues the background render the first time: it claims the render job and fires it via `after()` without blocking the response, so the very first read after an entry is normally `share.status: "pending"`. See [Pre-generated Pick'em entry images](./pickem-entry-images.md) for the full pipeline, retry/cleanup behavior, and required env vars.

The skill now makes image delivery a separate required post-confirmation step: wait for `share.status` to leave `"pending"` (a handful of short polling retries, not a busy loop), then GET and validate the image, attach it through the current reply surface (uploading bytes if required), or use the exact Markdown on surfaces that render images. A link preview is not a substitute for an X media attachment. If `status` stays `"pending"` past those retries, is `"failed"`, or media delivery otherwise fails after one fetch retry, send `share.fallbackText` verbatim (both direct links) as the whole confirmation — with no added sentence explaining the limitation (no "this surface can't attach images," no apology for the missing attachment). Seen in the wild: the skill used to be told to narrate that failure, and did, verbatim ("this reply surface cannot attach image media directly") — a technical status report nobody asked for. Staying honest about the entry itself (never claiming an attachment succeeded when it didn't) doesn't require narrating *why* it didn't. Never re-enter the contest to retry delivery.
...
Each `GET /api/bankr/contests/{id}/leaderboard` entry carries `ownerIdentity` in the same shape, resolved by the same rules for the current NFT owner. Only the returned page's owners are looked up: one Redis `MGET` against the shared `bankr:creator:v1:{address}` cache, thirdweb `getSocialProfiles` in parallel for misses only, and a 1.5s budget after which unresolved owners fall back to a shortened wallet.
